    ...
    <section id="testimonials" className="py-24 relative overflow-hidden">
      ...
      <div className="container relative">
        <motion.div
          ...
          viewport={{ once: true }}
        >
          <Badge className="mb-6 bg-gradient-to-r from-lime-500/10 to-orange-500/10 text-lime-600 border border-lime-500/20 px-5 py-2.5 text-sm font-semibold">
            <div className="w-5 h-5 rounded-md border border-lime-500/30 flex items-center justify-center mr-2">
        ...
        <div className="flex justify-end gap-2 mb-6 px-4">
          <motion.button
            onClick={() => scroll('left')}
            className="p-3 rounded-xl border border-border/50 bg-background/80 backdrop-blur-sm hover:bg-muted transition-all"
                ...
                <Card className="h-full bg-background/80 backdrop-blur-sm border border-border/50 hover:border-lime-500/30 hover:shadow-xl hover:shadow-lime-500/5 transition-all duration-300 group">
                  <CardContent className="p-8">
                    ...
                    <div className="flex items-center justify-between mb-6">
                      <div className="w-12 h-12 rounded-2xl bg-gradient-to-br from-lime-500/10 to-orange-500/10 border border-lime-500/20 flex items-center justify-center group-hover:scale-110 transition-transform">
                        <Quote className="h-5 w-5 text-lime-600" strokeWidth={1.5} />
                      </div>
                      <RatingIndicator rating={testimonial.rating} />
                    </div>

                    {/* Quote */}
                    <p className="text-lg text-foreground/80 mb-8 leading-relaxed line-clamp-4">
                      "{testimonial.quote}"
                    </p>

                    {/* Author - Premium card style */}
                    <div className="flex items-center gap-4 pt-6 border-t border-border/50">
                      <Avatar className="h-14 w-14 border-2 border-lime-500/20 ring-4 ring-lime-500/5">
                        <AvatarFallback className="bg-gradient-to-br from-lime-500/20 to-orange-500/20 text-lime-600 font-bold text-lg">
                          {testimonial.avatar}
                        </AvatarFallback>
                      </Avatar>
                      <div>
          ...
          {testimonials.map((_, index) => (
            <div
              ...
              className={`w-2 h-2 rounded-full transition-all ${
                Math.floor(scrollPosition / 400) % testimonials.length === index
                  ? 'bg-primary w-6'
